				...
				if (top_h[curr_head] >= 0) {
					...
					*/
					...
					if ((threadIdx.x % thread_per_head) == 0) {
						...
						if (blockIdx.x == 0) {
							if (threadIdx.x == 0) {
								printf("i = %d | indices = %f | cur_pos = %d\n", 
									i[curr_head], 
									dci_inst->indices[i[curr_head] * (dci_inst->num_points) + blockIdx.x * points_per_block].key,
									cur_pos[i[curr_head]]
								);
							}
						}
						...
						*/
						...
						if ((cur_pos[i[curr_head]] < 0) && (cur_pos[i[curr_head]] > -blockDim.x)) {
							...
							if (blockIdx.x == 0) {
								if (curr_head == 0) {
									printf("i = %d | position = %d | situation 1\n", i[curr_head], position[curr_head]);
								}
							}
							...
							if (blockIdx.x == 0) {
								if (curr_head == 0) {
									printf("i = %d | position = %d | situation 2\n", i[curr_head], position[curr_head]);
								}
							}
						} else {
							...
							if (blockIdx.x == 0) {
								if (curr_head == 0) {
									printf("i = %d | position = %d | situation 3\n", i[curr_head], position[curr_head]);
								}
							}
						}
						...
						if (blockIdx.x == 0) {
							if (threadIdx.x == 0) {
								printf("\n");
								for (int ch = 0; ch < num_heads; ch++) {
									printf("curr_head = %d | i = %d | position %d\n", ch, i[ch], position[ch]);
								}
							}
						}
