...
std::vector<torch::Tensor> py_dci_multi_query(std::vector<py::handle> py_dci_inst_wrapper, const int dim,  
    ...
    for (unsigned int i = 0; i < py_query.size(); i++) {
        calcs.push_back(std::async(py_dci_query, py_dci_inst_wrapper[i], dim, num_head_list[i], num_queries,
            py_query[i], num_neighbours, blind, num_outer_iterations, max_num_candidates, 
